...
<div class="card">
    <h5 class="card-header">Edit Why Choose</h5>
    <div class="card-body">
      <form method="post" action="{{route('whychoose.update')}}" enctype="multipart/form-data">
        {{csrf_field()}}
        ...
        <div class="form-group">
          ...
          <textarea class="form-control" id="contentData" rows="6" cols="20"  name="content">{{$whychoose->content}}</textarea>
          @error('main_heading')
        ...
        <div class="form-group">
          ...
          <img src="{{url('public/frontend/images/banner/'.$whychoose->image)}}" alt="Banner Image" width="200px" height="200px">
          @error('image')
          <span class="text-danger">{{$message}}</span>
          @enderror
        </div>
        ...
        <div class="form-group">
          ...
          <select name="status" class="form-control">
            ...
            <option value="0" {{(($whychoose->status=='0') ? 'selected' : '')}}>Inactive</option>
          </select>
          @error('status')
          <span class="text-danger">{{$message}}</span>
          @enderror
