...
@section('title','E-SHOP || State Create')
      ...
      <form method="post" action="{{route('state.store')}}">
        ...
        <div class="form-group">
          ...
          <select name="country_id" class="form-control">
            ...
            @foreach($clist as $cc):
            <option value="{{ $cc->id }}">{{ $cc->country_name }}</option>
            @endforeach;
          </select>
          @error('country_id')
        ...
        <input id="inputTitle" type="text" name="state_name" placeholder="Enter State Name"  value="{{old('state_name')}}" class="form-control">
        @error('state_name')
        <span class="text-danger">{{$message}}</span>
        @enderror
        </div>
